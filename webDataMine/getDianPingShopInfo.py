...
def getDianPingShopInfo(line):
    ...
    print(shopUrl)
    web_data = requests.get(shopUrl, headers=headers, proxies=random.choice(resource.PROXIES))
    ...
    try:
        ...
        shoptele = '未提供电话'
    commentData = requests.get(CommentUrl+str(1), headers=headers, proxies=random.choice(resource.PROXIES))
    ...
    for i in range(int(pagesOfComment)):
        ...
        CommentPageData = requests.get(CommentPageUrl , headers=headers, proxies=random.choice(resource.PROXIES))
